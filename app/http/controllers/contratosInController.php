<?php
require_once MODELS . 'ContratosIn.php';
require_once MODELS . 'StatusCMIN.php';
require_once MODELS . 'Rechazos.php';

class contratosInController
{

    public function index()
    {
        require_once VIEWS . 'contratosIn/index.php';
    }

    public function indexR()
    {
        require_once VIEWS . 'contratosIn/indexR.php';
    }

    function getHeaderCMIN()
    {
        ...
        switch ($codigoL) {
            ...
            case "5":
                ...
                break;
            case "CE":
                $where = "and step1 = 'X' and step2 = 'X' and step3 = 'X'";
                break;
        }
        ...
        if ($tipo == "1") {
            ...
            if(!empty($items) && $sum == 1) {
                foreach ($items as $key => $value) {
                    ...
                    $numR = new ContratosIn();
                    $numRowsXL = $numR->numPxL($poNumber);
                    //var_dump($numRowsXL);
                    $cmIN = new StatusCMIN();
                    $poNR = $cmIN->getCM1($poNumber);
                    if ($poNR == FALSE){
                        $cmIN->setPoNnumber($poNumber);
                        $save = $cmIN->saveCMIN();
                    }
                    $poNR2 = $cmIN->getCM2($poNumber,$where);
                ...
                foreach ($item as $key => $value) {
                    ...
                    if ($data['RelStatus'] == 'X') {
                        $cmIN->setStep1('X');
                        $update = $cmIN->updateCMIN($poNumber);
                    }
    ...
    function liberaContrato()
    {
        ...
        if (!empty($array)) {
            ...
            $cmIN = new StatusCMIN();
            $cmIN->updateCMINL($orden_compra,$step);
