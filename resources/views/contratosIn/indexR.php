...
<div class="container-fluid px-4">
    ...
    <h1 class="mt-4">Contratos Indirectos Rechazados</h1>
    <div class="mb-3 divS" >
        </br>
...
$( "#envRechazo" ).on( "click", function() {
    ...
    rechazarContrato(numPedido,txtRechazo);
});

function rechazarContrato(numPedido,txtRechazo) {
    ...
    if (selected.length === 0) {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                if (rolL == 'UsuarioMC') {
                    codigoL = $("#codigo").val();
                    setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(codigoL,'CI');}, 4000);
                }else{
                    codigoL =  "<?php echo $codigo_liberacion ?>";
                    setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(codigoL,'CI');}, 4000);
                }
...
$( document ).on( "click", ".detalleP", function() {
    ...
    $.ajax({
    url: "<?= URL ?>contratosIn/getItemsCM",
    type: "POST",
    cache: false,
    data: {
        numPedido: numPedido
