...
<div class="container-fluid px-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body" id="cardBody">
            ...
            <div class="table-responsive" id="divPedidosCMIN">
...
function showTableP(numPedido,grupoL,tipo) {
    ...
    $("#divPedidosCMIN").append(loadLabC);
    
    $.ajax({
        url: "<?= URL ?>contratosIn/getHeaderCMIN",
        type: "POST",
        cache: false,
        data: {
            codigoL: codigoL,
        ...
        success: function(response) {
            ...
            if (numRows >0) {
                ...
                $("#divPedidosCMIN").append("<div class='card bg-warning text-dark shadow'>No hay ordenes pendientes a liberar</div>");
                $(".cssload-jar").remove();
...
function rechazarContrato(numPedido,txtRechazo,coLi) {
    ...
    var url = "<?= URL ?>contratosIn/rechazoContrato";
    $.ajax({
        type: "POST",
        url: url,
        data: {
        ...
        success: function(data) {
            ...
            if (jsonData.TYPE == "OK") {
                ...
                //setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(coLi,'CI','1');}, 2000);
            } else {
                Swal.fire(jsonData.TYPE + ': ' + jsonData.MESSAGE);
                $("#rechazoModal").modal("hide");
            }
            ...
            if (rolL == 'UsuarioMC') {
                codigoL = $("#codigo").val();
                setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(codigoL,'CI','1');}, 4000);
            }else{
                codigoL =  "<?php echo $codigo_liberacion ?>";
                setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(codigoL,'CI','1');}, 4000);
            }
...
$( document ).on( "click", ".liberar", function() {
    ...
    swalWithBootstrapButtons.fire({
        ...
        if (result.isConfirmed) {
            ...
            var url = "<?= URL ?>contratosIn/liberaContrato";
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (jsonData.TYPE == "OK") {
                        ...
                        //codigoL =  "<?php echo $codigo_liberacion ?>";
                        //setTimeout(function() {$("#divPedidosCMIN").empty();showTableP(codigoL,'CI','1');}, 2000);
                        rolL = "<?php echo $rol ?>";
...
$( document ).on( "click", ".checkAllCMIN", function() {
    if (this.checked) {
        $(".checkboxesCMIN").prop("checked", true);
    } else {
        $(".checkboxesCMIN").prop("checked", false);
    }
});

$( document ).on( "click", ".liberaAllCMIN", function() {

    var selected = '';
    $('[name="checkboxesCMIN[]"]').each(function() {
        if (this.checked) {
            selected += $(this).val() + ',';
        }
    });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (jsonData.TYPE == "OK") {
                                ...
                                $('.lbNokCMIN').append(div);
                                $('.lbNokCMIN').removeAttr('hidden');
                            }
                        }
                    });
                }
...
$( document ).on( "click", ".detalleP", function() {
    ...
    $.ajax({
    url: "<?= URL ?>contratosIn/getItemsCMIN",
    type: "POST",
    cache: false,
    data: {
        numPedido: numPedido
